...
package com.charon.video.backup;

import com.charon.video.R;
import com.charon.video.backup.VideoViewBF.OnStateChangeListener.State;
 ...
 */
public class VideoViewBF extends SurfaceView implements
    ...
    /**
     * Unused, This have been used in OPlayer.
     */
    ...
    private MediaPlayer.OnErrorListener mErrorListener = new MediaPlayer.OnErrorListener() {
        public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
            ...
            Log.e(TAG, "onError : the error code is : " + framework_err);
            stateChange(State.STATE_ERROR);

            if (MediaPlayer.MEDIA_INFO_VIDEO_TRACK_LAGGING == framework_err) {
                Log.e(TAG,
                        "onError : The video is too complex for the decoder: it can't decode frames fast enough.");
            } else if (MediaPlayer.MEDIA_ERROR_NOT_VALID_FOR_PROGRESSIVE_PLAYBACK == framework_err) {
                Log.e(TAG, "onError : A bad video file.");
            }
            if (mMediaController != null) {
                mMediaController.hide();
            }
            ...
            // If return false or have not set OnErrorListener then
            // OnCompletionListener will be casued.
            /*
        ...
        public boolean onInfo(MediaPlayer mp, int what, int extra) {
            if (mOnInfoListener != null) {
                ...
                if (what == MediaPlayer.MEDIA_INFO_BUFFERING_START) {
                    Log.w(TAG, "start buffering and pause the mediaplayer");
                    if (isPlaying()) {
                        mMediaPlayer.pause();
                        mIsBuffering = true;
                    }
                    ...
                    Log.w(TAG, "stop buffering, start to play");
                    if (mIsBuffering) {
                        mMediaPlayer.start();
                        mIsBuffering = false;
                    }
    ...
    public VideoViewBF(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        initVideoView(context);
    }

    public VideoViewBF(Context context, AttributeSet attrs) {
        super(context, attrs);
        initVideoView(context);
    }

    public VideoViewBF(Context context) {
        super(context);
        initVideoView(context);
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        // Can control the controller use this way, but if the controller is
        // full screen you must control it throw the controller inside.
    ...
    public void suspend() {
        if (isInPlaybackState()) {
            ...
            // openVideo, but this may have a problem is that the media player
            // may still buffering the video, this may lost many traffic of the
            // user.
            pause();
